"""End-to-end test script for Planner + DAG pattern.

Uses the shared main module functions for consistency with CLI.
Run with: uv run python tests/planner/e2e_test.py
Output: output/test_planner_*.md
"""
...
from youtube_agent_planner.application.main import process_request
...
logging.getLogger("youtube_agent_planner").setLevel(logging.INFO)
...
VIDEO_IDS_REQUIRED = False  # Planner pattern may not include video sources in output
...
Save the results to test_planner_pork_loin.md"""

    print("\n" + "=" * 60)
    print("PLANNER + DAG PATTERN E2E TEST")
    print("=" * 60)
    print("USER REQUEST:")
    print(request)
    print("=" * 60 + "\n")
    ...
    try:
        ...
        print("=" * 60)
        print(response if response else "No response (check logs)")

        # Verify output
        output_path = Path("output/test_planner_pork_loin.md")
        print("\n" + "=" * 60)
        print("VERIFICATION:")
        print("=" * 60)
